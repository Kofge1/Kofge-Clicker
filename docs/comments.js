(() => {
  ...
  const strings = language === "ru"
    ? {
        ...
        oneComment: "опубликованный отзыв",
        manyComments: "опубликованных отзывов",
        sending: "Отправляем…",
        ...
        version: "Версия",
        developerReply: "Ответ разработчика"
      }
    : {
        ...
        oneComment: "published review",
        manyComments: "published reviews",
        sending: "Sending…",
  ...
  function renderComments(comments) {
    ...
    for (const comment of comments) {
      ...
      if (comment.authorReply) {
        ...
        replyTitle.textContent = strings.developerReply;
        const replyBody = document.createElement("p");
        replyBody.textContent = comment.authorReply;
        reply.append(replyTitle, replyBody);
  ...
  async function submitReview(event) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.error || strings.genericError);
      }
